...
def send_message(school_code, sender_email, recipient_email, message):
...
# ----- DASHBOARD -----
    ...
    with st.sidebar:
        ...
        # Fix: Ensure menu_index is within valid range
                        ...
                        with st.container():
                            ...
                            with col3:
                                if st.button("💬 Chat", key=f"chat_friend_{friend_email}"):
                                    ...
                                    # Update menu index to Chat
                                    chat_option = next((i for i, opt in enumerate(options) if opt.startswith("Chat")), 0)
                                    st.session_state.menu_index = chat_option
                                    st.rerun()
                            st.divider()
            else:
                            ...
                            with col3:
                                ...
                                with col_b:
                                    if st.button("❌ Decline", key=f"decline_{req['id']}"):
                                        req['status'] = 'declined'
                                        save_school_data(school_code, "friend_requests.json", load_school_data(school_code, "friend_requests.json", []))
                                        st.rerun()
                            st.divider()
            else:
                ...
                if other_user:
                    ...
                    with st.form("send_message"):
                        ...
                        if st.form_submit_button("📤 Send", use_container_width=True):
                            if message:
                                send_message(school_code, user['email'], other_email, message)
                                st.rerun()
            else:
    ...
    # ----- TEACHERS (admin only) -----
        ...
        tab1, tab2 = st.tabs(["✨ Create Code", "👥 Active Teachers"])
                    ...
                    with st.expander(f"{t['name']} - `{t['code']}`"):
                        ...
                        if t.get('used_by_list'):
                            ...
                            for teacher in t['used_by_list']:
                                st.write(f"- {teacher['name']} ({teacher['email']})")
            else:
                st.info("No teacher codes created")
